...
<div class="shopping_cart">
 ...
 foreach($cartproducts as $prd){
	// print_r($cartproducts); 
	 if($prd->bottle_size!='')
	 {
	  $nag="select * from tb_store_value where spid=$prd->prod_id and size=$prd->bottle_size";
	 }
	 else 
	 {
	  $nag="select * from tb_store_value where spid=$prd->prod_id and size=$prd->size" ;
	 }
	 $gfdg=$callConfig->getRow($nag);
	 $totalweight=$gfdg->weight;
	   if($prd->p_img!='' )
							{
               ...
               <img src="<?php echo SITEURL; ?>/<?php echo $src; ?>"  style="width:55px; height:56px;"/></a></span>
                  ...
                  <div class="product1_name">
                     <?php echo stripslashes($prd->prod_name)?>
                         ...
                         <?php if($prd->type=='monthly'){?>( Monthly Special )<?php }?>
                         
                          
                        </div><!--product_color-->

                       <?php if($prd->cart_cid=='1') { ?>
...
<input type="submit" value="UPDATE" name="submit" style="padding:6px 10px; background-color:#8ebe29; border:none; font-family: 'Roboto', sans-serif; font-size:10px; margin-top:10px; -webkit-border-radius: 3px;	-moz-border-radius: 3px; border-radius: 3px; color:#FFF; cursor:pointer;"/>
...
?>
                  ...
                  <div class="product1_qty"></div>
				      ...
				      <div class="clearcart1_btn">
                
                    <a href="productpage.php?id=<?php echo $prd->cart_cid; ?>"> Continue Shopping</a></div>
