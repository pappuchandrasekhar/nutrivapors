...
$gettmpcart32="Select * from tb_cart  where temprandid='".$var_temp_session."'";
 ...
 foreach($gettmprowcartres32 as $prd)
		...
		echo   $sql_orders12="update tb_store_value set quantity='".$quantitiesnew."' where spid='".$prd->prod_id."' and size='".$prd->bottle_size."'"; 
		
			$sqlordins_res2=mysql_query($sql_orders12);
			
			
			$nag="select * from tb_store_value where spid='".$prd->prod_id."' and  size='".$prd->bottle_size."'";
			$ex=$callConfig->getRow($nag);
			
			echo  $dem=$ex->quantity;  
