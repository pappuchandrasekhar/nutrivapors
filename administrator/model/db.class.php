...
class configClass
{
		...
		if($where!="")
		echo $query.=DBWHR.$where." IN('".$field_ids."')";
		$res = configClass::executeQuery($query);
...
/* new code for image size reduction  */




function freealbumImagesUpload($jnt,$imagename,$path,$thumb_path,$hdnimage,$width,$height)
		{
			...
			if($_FILES[$imagename]['name']!="") 
			{
				...
				move_uploaded_file($_FILES[$imagename]['tmp_name'],$path.$name);
				$thumb_dir=$thumb_path;
				$original_dir=$path;
				configClass::createthumbImages($original_dir.$name,$thumb_dir, $width,$height);
				if(file_exists($thumb_path.$hdnimage)) @unlink($thumb_path.$hdnimage);
				if(file_exists($path.$hdnimage)) @unlink($path.$hdnimage);	
			}
			else 
			{
...
function freealbumImagesUpload3($jnt,$imagename,$path,$thumb_path,$hdnimage,$width,$height)
		{
			...
			if($_FILES[$imagename]['name']!="") 
			{
				...
				$thumb_dir=$thumb_path;
				$original_dir=$path;
				configClass::createthumbImages3($original_dir.$name,$thumb_dir, $width,$height);
				if(file_exists($thumb_path.$hdnimage)) @unlink($thumb_path.$hdnimage);
				if(file_exists($path.$hdnimage)) @unlink($path.$hdnimage);	
			}
			else 
	...
	function createthumbImages2($srcpath,$dir,$width,$height)
	{
	...
	function freealbumImagesUpload2($jnt,$imagename,$path,$thumb_path,$hdnimage,$width,$height)
		{	
			 $thumb_dir=$thumb_path;
			 $original_dir=$path.$imagename;
			 list($width, $height, $type, $attr) = getimagesize($original_dir);
			 if($width>175)
			 {
				 configClass::createthumbImages3($original_dir,$thumb_dir,$width,$height);
			 }
			 else
			 {
			 	configClass::createthumbImages($original_dir,$thumb_dir, $width,$height);
			 }
			 return $imagename;
		}	
...
function createthumbImages3($srcpath,$dir,$width,$height)
	{
...
function createthumbImages($srcpath,$dir,$width,$height)
	{
...
function random_code()
{
	...
	return time().rand(1,time()).$rand.$salt;
		...
		function remove_special_symbols2($string)
		{
		//Array of special charecters you want to replace
		//$special = array('^','>','<','`','~','\'','"','\\','_','!','@','#','$','%','&','*','(',')','=','+','{','}','[',']','.',',','/','?','|','-',':',';',' '); //here you can add as many char. you want
		$special = array('^','>','<','`','~','\'','"','\\','_','!','@','#','$','%','&','*','(',')','=','-','{','}','[',']','.',',','/','?','|',':',';',' '); //here you can add as many char. you want
