...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
	...
	
	{
		...
		$quantity=1;
			...
			{
				//insert
				$fieldnames=array('d_type'=>$_POST['d_typeid'][$i],'size'=>$_POST['sid'][$i],'real_price'=>$_POST['rs'][$i],'bulk_price'=>$_POST['brs'][$i],'weight'=>$_POST['wrs'][$i],'spid'=>$res ,'quantity'=>$quantity);
				//print_r($fieldnames); exit;
...
echo   $query="update tb_store_value SET d_type='".$_POST['d_typeid'][$sid_keys[$i]]."',size='".$_POST['sid'][$sid_keys[$i]]."',real_price='".$_POST['rs'][$i]."',bulk_price='".$_POST['brs'][$i]."',weight='".$_POST['wrs'][$i]."',quantity='".$_POST['qns'][$i]."',spid='".$post['hdn_id']."' WHERE spid='".$post['hdn_id']."' and id='".$sid_keys[$i]."'";
