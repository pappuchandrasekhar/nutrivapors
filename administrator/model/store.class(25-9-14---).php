...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
   ...
   if($width>300)
   {
   $prodimage =$callConfig->freeimageUploadcomncode("cat",'image',$path,$path."thumbs/",$hdn_file_name,179,181);
...
//		}
	...
	$fieldnames=array('scid'=>$post['scid'],'sscid'=>$_POST['sscid'],'bid'=>$post['bid'],'prodtitle'=>mysql_real_escape_string($post['prodtitle']),'prodsizes'=>$sizepro,'prodsizes'=>$_POST['prodsizes'],'bulkpacketcount'=>$post['bulkpacketcount'],'prodtitle_slug'=>$titleslug,'bigtext'=>mysql_real_escape_string($post['content']),'image'=>$image,'image1'=>$image1,'image2'=>$image2,'image3'=>$image3,'color'=>$post['color'],'colorstab'=>$colors, 'featuredlisting'=>$_POST['featured'],'nicotinecontent'=>$_POST['nicotinecontent'],'finish'=>$_POST['finish'],'real_price'=>$_POST['real_price'],'rank'=>$_POST['rank'],'title_slug'=>$titleslug,'meta_keyword'=>mysql_real_escape_string($post['meta_keyword']),'meta_desc'=>mysql_real_escape_string($post['meta_desc']),'lotnumber'=>mysql_real_escape_string($post['lot']),
...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
	...
	
	{
		...
		for($i=0;$i<count($_POST['sid']);$i++)
		{
			...
			{
				//insert
				$fieldnames=array('size'=>$_POST['sid'][$i],'real_price'=>$_POST['rs'][$i],'bulk_price'=>$_POST['brs'][$i],'weight'=>$_POST['wrs'][$i],'spid'=>$res);
				//print_r($fieldnames); exit;
...
$query="update tb_store_value SET size='".$_POST['sid'][$sid_keys[$i]]."',real_price='".$_POST['rs'][$i]."',bulk_price='".$_POST['brs'][$i]."',weight='".$_POST['wrs'][$i]."',spid='".$post['hdn_id']."' WHERE spid='".$post['hdn_id']."' and id='".$sid_keys[$i]."'";
mysql_query($query);
