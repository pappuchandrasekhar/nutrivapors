...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
	...
	
	{
		...
		for($i=0;$i<count($_POST['sid']);$i++)
		{
			...
			{
				//insert
				$fieldnames=array('size'=>$_POST['sid'][$i],'real_price'=>$_POST['rs'][$i],'bulk_price'=>$_POST['brs'][$i],'spid'=>$res);
				//print_r($fieldnames); exit;
...
$query="update tb_store_value SET size='".$_POST['sid'][$sid_keys[$i]]."',real_price='".$_POST['rs'][$i]."',bulk_price='".$_POST['brs'][$i]."',spid='".$post['hdn_id']."' WHERE spid='".$post['hdn_id']."' and id='".$sid_keys[$i]."'";
mysql_query($query);
