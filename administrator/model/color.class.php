...
{ 
 //  door color //
  ...
  function getAlldoorcolorListCount($where)
  {
	...
	if($where!="")
	$whr=" status='".$where."'";
	$query=$callConfig->selectQuery(TPREFIX.TBL_DOOR_COLOR,'dcid',$whr,'','','');
	return $callConfig->getCount($query);
  } 
  function getdoorcolorData($id)
  {
	global $callConfig;
	$query=$callConfig->selectQuery(TPREFIX.TBL_DOOR_COLOR,'*','dcid='.$id,'','','');
	return $callConfig->getRow($query);
 }
 
 
 function insertdoorcolor($post)
	{
		global $callConfig;
		/*if($post['reloc']=='color'){
...
// print_r($fieldnames); 
 $res=$callConfig->insertRecord(TPREFIX.TBL_DOOR_COLOR,$fieldnames);
	if($res!="")
	{
		sitesettingsClass::recentActivities('colors >> color created successfully on >> '.DATE_TIME_FORMAT.'','g');
		$callConfig->headerRedirect("index.php?option=com_color&err=color created successfully");
	}
	else
	{
		sitesettingsClass::recentActivities('colors >> color creation failed on >> '.DATE_TIME_FORMAT.'','e');
		$callConfig->headerRedirect("index.php?option=com_color&ferr=color color creation failed");
	}
	}
 ...
 function updatedoorcolor($post)

	{
        ...
        $res=$callConfig->updateRecord(TPREFIX.TBL_DOOR_COLOR,$fieldnames,'dcid',$post['hdn_id']);
...

		{
			...
			$callConfig->headerRedirect("index.php?option=com_color&err=colors updated successfully");
...

		{
			...
			$callConfig->headerRedirect("index.php?option=com_color&ferr=colors updation failed");

		}

	}
	
	function doorcolorDelete($id)

	{

	global $callConfig;
	$query=$callConfig->selectQuery(TPREFIX.TBL_DOOR_COLOR,'image','dcid='.$id,'','','');
	$imageres = $callConfig->getRow($query);
	$callConfig->imageCommonUnlink("../uploads/panel/","../uploads/panel/thumbs/",$imageres->image);
	$res=$callConfig->deleteRecord(TPREFIX.TBL_DOOR_COLOR,'dcid',$id);
	if($res==1)

	{
		sitesettingsClass::recentActivities('colors >> color deleted successfully on >> '.DATE_TIME_FORMAT.'','e');
		$callConfig->headerRedirect("index.php?option=com_color&err=color deleted successfully");
...

	{
		...
		sitesettingsClass::recentActivities('colors >>color deletion failed on >> '.DATE_TIME_FORMAT.'','e');

		$callConfig->headerRedirect("index.php?option=com_color&ferr=color deletion failed");
  ...
  function getAlldoordimensionsList($where,$sortfield,$order,$start,$limit)
  {
	...
	if($where!="")
	$whr=" status='".$where."'";
	$query=$callConfig->selectQuery(TPREFIX.TBL_DIMENSIONS,'*',$whr,$order,$start,$limit);
	return $callConfig->getAllRows($query);
  } 
