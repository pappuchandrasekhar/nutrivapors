...
$path="../uploads/store/category/";
//$path2="../uploads/store/category/small/";
...
$image = $callConfig->freeimageUploadcomncode("cat",'image',"../uploads/store/category/","../uploads/store/category/thumbs/",$post['hdn_image'],208,151);
...
// end store category //
 ...
 function getStoreSubCategory_D($id)
  {
	global $callConfig;
	 $query=$callConfig->selectQuery(TPREFIX.TBL_STORESUBCATEGORY,'*','scid='.$id,'','','');
	return $callConfig->getAllRows($query);
 }
...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
	...
	function insertProducts($post)
	{
		...
		/*$prodimage = $callConfig->freeimageUploadcomncode('prod','image',"../uploads/store/products/","../uploads/store/products/thumbs/",$post['hdn_image'],179,181);
   ...
   //if($width>300)
   //{
   //$prodimage =$callConfig->freeimageUploadcomncode("cat",'image',$path,$path."thumbs/",$hdn_image,179,181);
   //$callConfig->freealbumImagesUpload2('cat',$prodimage,$path,$path,$hdn_file_name,'179',181);
   //$image=$prodimage;
   
   
   $image = $callConfig->freeimageUploadcomncode("cat","image","../uploads/$path/","../uploads/$path/thumbs/",$post['hdn_image'],161,161);
   
    //$images = $callConfig->freeimageUploadcomncode("cat","image","../uploads/$path/","../uploads/$path/thumbs/",$post['hdn_image'],175,180);
   
   //print_r($image);
  //}
 // else
 // {
	  //$image = $callConfig->freealbumImagesUpload('cat','image',$path,$path."/thumbs/",$post['hdn_image'],179,181);
  ...
  /*image1*/
   ...
   $image1 = $callConfig->freeimageUploadcomncode('prod','image2',"../uploads/store/products/","../uploads/store/products/thumbs/",$post['hdn_image2'],161,161);
		$image2 = $callConfig->freeimageUploadcomncode('prod','image3',"../uploads/store/products/","../uploads/store/products/thumbs/",$post['hdn_image3'],160,161);
		$image3 = $callConfig->freeimageUploadcomncode('prod','image4',"../uploads/store/products/","../uploads/store/products/thumbs/",$post['hdn_image4'],160,161);
		//if($post['offer']=="no"){
//			$oldprice="";
//			$newprice=$post['newprice'];
//		} 
...
//		}
    ...
    $colors = implode(",",$lang);
	 ...
	 //print_r($vcolors);exit;
    $colors1 = implode(",",$vcolors);
	
	$vsize= $_POST['vsizes'];
	//print_r($vsize);exit;
    $vsize1 = implode(",",$vsize);
	
	
	//$sizes= $_POST['size'];
    //$sizepro= implode(",",$sizes);
	/*'disp_order'=>$post['display_order'],*/
	$fieldnames=array('scid'=>$post['scid'],'sscid'=>$_POST['sscid'],'d_type'=>$_POST['d_type'],'bid'=>$post['bid'],'prodtitle'=>mysql_real_escape_string($post['prodtitle']),'prodsizes'=>$sizepro,'prodsizes'=>$_POST['prodsizes'],'bulkpacketcount'=>$post['bulkpacketcount'],'prodtitle_slug'=>$titleslug,'bigtext'=>mysql_real_escape_string($post['content']),'image'=>$image,'image1'=>$image1,'image2'=>$image2,'image3'=>$image3,'color'=>$post['color'],'colorstab'=>$colors, 'featuredlisting'=>$_POST['featured'],'nicotinecontent'=>$_POST['nicotinecontent'],'finish'=>$_POST['finish'],'real_price'=>$_POST['real_price'],'vcolors'=>$colors1,'vsizes'=>$vsize1,'rank'=>$_POST['rank'],'title_slug'=>$titleslug,'meta_keyword'=>mysql_real_escape_string($post['meta_keyword']),'meta_desc'=>mysql_real_escape_string($post['meta_desc']),'lotnumber'=>mysql_real_escape_string($post['lot']),
...
'waistband'=>mysql_real_escape_string($post['wai']),'status'=>$post['status']);
	...
	
	{
		...
		$quantity=1;
			...
			{
				//insert
				$fieldnames=array('size'=>$_POST['sid'][$i],'real_price'=>$_POST['rs'][$i],'bulk_price'=>$_POST['brs'][$i],'weight'=>$_POST['wrs'][$i],'spid'=>$res ,'quantity'=>$quantity);
				//print_r($fieldnames); exit;
		        $res2=$callConfig->insertRecord(TPREFIX.TBL_STORE_VALUE,$fieldnames);
	...
	$image = $callConfig->freeimageUploadcomncode("cat","image","../uploads/$path/","../uploads/$path/thumbs/",$post['hdn_image'],179,181);
  ...
  $image1 = $callConfig->freeimageUploadcomncode('prod','image2',"../uploads/store/products/","../uploads/store/products/thumbs/",$post['hdn_image2'],161,161);
	...
	$fieldnames=array('scid'=>$post['scid'],'sscid'=>$_POST['sscid'],'d_type'=>$_POST['d_type'],'bid'=>$post['bid'],'prodtitle'=>mysql_real_escape_string($post['prodtitle']),'prodtitle_slug'=>$titleslug,'bigtext'=>mysql_real_escape_string($post['content']),'bulkpacketcount'=>$post['bulkpacketcount'],'image'=>$image,'image1'=>$image1,'image2'=>$image2,'image3'=>$image3,'color'=>$post['color'],'colorstab'=>$colors,'real_price'=>$post['real_price'],'nicotinecontent'=>$_POST['nicotinecontent'],'finish'=>$post['finish'],'rank'=>$_POST['rank'], 'featuredlisting'=>$_POST['featured'],'prodsizes'=>$_POST['prodsizes'],'title_slug'=>$titleslug,'meta_keyword'=>mysql_real_escape_string($post['meta_keyword']),'vcolors'=>$colors1,'vsizes'=>$vsize1,'meta_desc'=>mysql_real_escape_string($post['meta_desc']),'lotnumber'=>mysql_real_escape_string($post['lot']),
		...
		for($i=0;$i<count($_POST['sid']);$i++)
		{
			...
			if($_POST['sid'][$i]!='' || $_POST['rs'][$i])
			{
		        ...
		        //print_r($fieldnames); 
  				...
  				//$res2=$callConfig->updateRecord(TPREFIX.TBL_STORE_VALUE,$fieldnames,'spid='.$post['hdn_id'].' & id='.$sid_keys[$i],'');
				//print_r($res2);,'bulk_price'=>$_POST['brs'][$i]
  $query="update tb_store_value SET size='".$_POST['sid'][$sid_keys[$i]]."',real_price='".$_POST['rs'][$i]."',bulk_price='".$_POST['brs'][$i]."',weight='".$_POST['wrs'][$i]."',quantity='".$_POST['qns'][$i]."',spid='".$post['hdn_id']."' WHERE spid='".$post['hdn_id']."' and id='".$sid_keys[$i]."'";
