	 ...
	 <table width="100%" border="0" cellpadding="0" cellspacing="0"  id="dataTable" class="table table-bordered table-condensed table-hover table-striped">
          <thead>
            <tr height="40">
              ...
              <!-- <th width="131" align="left" valign="middle">Rating</th>-->
            
			   <th width="300" align="left" valign="middle" >Image</th>
...
<form action="index.php?option=com_storeproducts_insert" method="post" id="frmCreatestate" name="frmCreatestate" class="middle_form" enctype="multipart/form-data" onsubmit="return validate();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" >
           ...
           <!--    <tr><td colspan="5" height="7"></td></tr>-->
               ...
               <tr>
				...
				<select name="scid" id="scid" class="select_medium required" onchange="showCustomer(this.value)" style="margin-left:8px;">
			  ...
			  function showCustomer(str)
			  {
				 if(str=='1')
					 {
						document.getElementById('EJuice').style.display='block';
						 document.getElementById('VapePens').style.display='none';
					 }
					 else if(str=='2'){
							document.getElementById('EJuice').style.display='none';
							document.getElementById('VapePens').style.display='block';
						 }
        ...
        <div id="EJuice" style="display:none;"> 
    <table>
       <tr> <td width="7%" align="left" class="caption-field"><label class="title">Nicotine Content:<span style=" color:#FF0000; font-size:14px">*</span></label></td>
        
        <td>
		
		
		<select name="nicotinecontent" id="nicotinecontent" class="select_medium">
         <?php /*?><option value="<?php echo  stripslashes($indivdata->nicotinecontent);?>"><?php echo  stripslashes($indivdata->nicotinecontent);?></option>
          <option value="select">Select Nicotine Content</option>
                ...
                <option value="<?php echo $i;?>mg"><?php echo $i;?> Mg</option>
                <?php }?><?php */?>
         ...
         <script type="text/javascript">
                for(var i=0;i<document.getElementById('nicotinecontent').length;i++)
                {
						if(document.getElementById('nicotinecontent').options[i].value=="<?php echo $indivdata->nicotinecontent ?>")
						{
						document.getElementById('nicotinecontent').options[i].selected=true
						}
                }			
                </script>
        ...
        <div id="VapePens"  style="display:none">
      <table>
      <tr>
        <td width="7%" align="left" class="caption-field"><label class="title">Colors:<span style="color:#FF0000; font-size:14px">*</span></label></td>
        
        <td >
		<input type="checkbox"  name="vcolors[]"  value="red"/>Red
        <input type="checkbox"  name="vcolors[]"  value="green"/>Green
         <input type="checkbox" name="vcolors[]"  value="Blue"/>blue
         <input type="checkbox" name="vcolors[]"  value="Yellow"/>Yellow
        ...
        <td width="7%" align="left" class="caption-field"><label class="title">Sizes:<span style=" color:#FF0000; font-size:14px">*</span></label></td>
        
        <td >
		<input type="checkbox" name="vsizes[]"  value="10"/>10
        <input type="checkbox" name="vsizes[]"  value="30"/>30
    ...
    <tr><td colspan="2" height="7"></td></tr>
         ...
         <?php /*?>  <tr> 
                <td width="6%" align="left" class="caption-field"><label class="title">Bulk Packet Count :</label></td>
                <td width="94%" align="left" valign="middle"><input name="bulkpacketcount" class="text_large required" type="text" value="<?php echo  stripslashes($indivdata->bulkpacketcount);?>" required="required" /></td>
	</tr><?php */?>
...
<?php
				 ...
				 {
                 $optin='';
				  $asqffds="select * from tb_store_value where spid='".$indivdata->spid."' "; 
         ...
         <tr>
			   <?php      $asq="select * from tb_store_value where spid='".$indivdata->spid."' "; 
				         $price=$callConfig->getRow($asq);
						 $sid_single = $price->id;
						
						 $dtsq="select * from tb_store_value where spid='".$indivdata->spid."' "; 
				         $dt_sq=$callConfig->getRow($dtsq);
						 $dtid_single = $dt_sq->id; 
				                   ?>
			  <td width="7%" align="left" class="caption-field"><label class="title">Size:<span style=" color:#FF0000; font-size:14px">*</span></label></td>
                ...
                <td width="100%" colspan="4" align="left" valign="middle">
				 ...
				 <?php /*?><strong>Name:</strong><input name="d_typeid[<?php print $dtid_single;?>]" class="inputtype_field_input" type="text" value="<?php echo  $dt_sq->d_type; ?>" required="required" /><?php */?>
                
				<strong>Name:</strong><input name="d_type" class="inputtype_field_input" type="text" value="<?php echo  $indivdata->d_type; ?>" required="required" />
				
              				 <strong>Value:</strong> <input name="sid[<?php print $sid_single;?>]" class="inputtype_field_input" type="text" value="<?php echo $price->size; ?>" required="required" />
             ...
             <script type="text/javascript">
                ...
                <input type="text" name="rs[]" id="rs0" style="width:70px" value="<?php echo $price->real_price;?>"/>
                <strong>Monthly Price:</strong>
               
                <input type="text" name="brs[]" id="brs0" style="width:70px"  value="<?php echo $price->bulk_price;?>"/>
  <strong>Weight(oz):</strong>
               
                <input type="text" name="wrs[]" id="wrs0" style="width:70px"  value="<?php echo $price->weight;?>"/>
                
                
                
                <strong>Quantity:</strong>
               
                <input type="text" name="qns[]" id="qns0" style="width:70px"  value="<?php echo $price->quantity;?>"/>
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style=" color:#FF0000; font-size:14px;margin-left:30px;">*</span>(If you want to add more sizes click below Plus button to Add ) </span></td>
     ...
     <div id='row<?=$i?>'><div style='border:1px solid #6DB5FF; width:850px; height:45px;padding:2px 2px 2px 2px;'><p>
	 
<?php /*?><input type='text' value="<?=$s_price->d_type?>" name='sid[<?php print $s_price->id;?>]' id='sid<?=$i?>' class='inputtype_field_input' /><?php */?>
	 
	  <input type='text' value="<?=$s_price->size?>" name='sid[<?php print $s_price->id;?>]' id='sid<?=$i?>' class='inputtype_field_input' />
    <!-- <select name='sid[<?php print $s_price->id;?>]' id='sid<?=$i?>' class='select_medium'><option value=''>Select</option><?php print $optin; ?></select>
     ...
     price:<input type='text' value="<?=$s_price->real_price?>" name='rs[]' id='rs<?=$i?>' class='inputtype_field_input' />&nbsp;&nbsp;Monthly price:<input type='text' value="<?=$s_price->bulk_price?>" name='brs[]' id='brs<?=$i?>' style="width:70px;" class='inputtype_field_input' />&nbsp;&nbsp;Weight(oz):<input type='text' value="<?=$s_price->weight?>" style="width:70px;" name='wrs[]' id='wrs<?=$i?>' class='inputtype_field_input' />&nbsp;&nbsp;Quantity: <input type='text' value="<?=$s_price->quantity?>" style="width:70px;" name='qns[]' id='qns<?=$i?>' class='inputtype_field_input' /></p>
     <div class='clearfix'></div><a href='javascript:void(0);' onClick='removeFormField("#row<?=$i?>"),deleterow("<?=$s_price->id?>");return false;'>
     <img src='allfiles/Delete-Button_06.png' alt='delete' style='position:relative;top:-56px;float:right;right:-35px' /></a></div><br></div>
...
$("#divTxt").append("<div id='row" + id + "'><div style='border:1px solid #6DB5FF;width:980px; height:45px;padding:2px 2px 2px 2px;'><p><input type='text' name='sid[]' id='sid"+id+"' class='inputtype_field_input' /><?php //print $optino; ?> price:<input type='text' name='rs[]' id='rs"+id+"' class='inputtype_field_input' />Monthly Price:<input type='text' name='brs[]' id='brs"+id+"' class='inputtype_field_input' />  Weight(oz):<input type='text' name='wrs[]' id='wrs"+id+"' class='inputtype_field_input' />Quantity:<input type='text' name='qns[]' id='qns"+id+"' class='inputtype_field_input' /> </p><div class='clearfix'></div><a href='#' onClick='removeFormField(\"#row" + id + "\"); return false;'><img src='allfiles/Delete-Button_06.png' alt='delete' style='position:relative;top:-56px;float:right;right:-35px' /></a></div><br></div>");
$("label").inFieldLabels(); 
	id = (id - 1) + 2;
	...
	<div id="txtHint"></div>
              ...
              <tr>
			   ...
			   <tr>
				...
				?> <span style="font-size:11px">(Image sizes: width:364px height:364px)</span></td>
				</tr>
