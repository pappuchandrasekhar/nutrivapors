...
if($_GET['action']=="delete"){
   $testimonialObj->testimonialDelete($_GET['id']);
}
if($_POST['admininsert']=="Submit"){
	echo "hai";
	
	//print_r($_POST);
   $testimonialObj->inserttestimonial($_POST);
}
if($_POST['admininsert']=="Update"){
   $testimonialObj->updatetestimonial($_POST);
}
if(isset($_GET['id']) && $_GET['id']!=""){
	//echo "hai";
   $hdn_value="Update";
   $indivdata=$testimonialObj->gettestimonialData($_GET['id']); 
...
$orderby="DESC";
$allforumlist=$testimonialObj->getAlltestimonialList('',$fldname,$orderby,$start,$limit);
$total=$testimonialObj->getAlltestimonialListCount('');
...
<div class="content">
	 <table width="100%" border="0" cellpadding="0" cellspacing="0"  id="dataTable" class="table table-bordered table-condensed table-hover table-striped">
          <thead>
			...
			<td  align="middle" valign="middle" ><?php echo stripslashes($allforum_list->title);?></td>
            <td  align="middle" valign="middle"><img src="../uploads/testimonial/category/<?php echo $allforum_list->image; ?>" height="60" width="60"/></td>
			<td align="middle" valign="middle"><?php echo $allforum_list->status;?></td>
			<td align="middle" valign="middle"x><a title="edit" href="index.php?option=com_testimonial_insert&id=<?php echo $allforum_list->id;?>"><img src="allfiles/icon_edit.png" alt="Edit" border="0"></a></td>
			<td align="middle" valign="middle">
			
			<?php if(($allforum_list->id=='3') || ($allforum_list->id=='4') || ($allforum_list->id=='5') || ($allforum_list->id=='6')) {?> default <?php } else {?>
            <a title="delete" href="#" onClick="var q = confirm('Are you sure you want to delete selected record?'); if (q) { window.location = 'index.php?option=com_testimonial&action=delete&id=<?php echo $allforum_list->id;?>'; return false;}"><img src="allfiles/icon_delete.png"  alt="Delete" border="0"/></a>
				<?php }?>		
			  </td>
			</tr>
			<?php
...
<script type="text/javascript">
	tinyMCE.init({
		...
		theme_advanced_buttons4 : "insertlayer,moveforward,movebackward,absolute,|,styleprops,|,cite,abbr,acronym,del,ins,attribs,|,visualchars,nonbreaking,template,pagebreak,retestimonialdraft",
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td align="left" valign="middle"> <h1><img src="allfiles/map.jpeg" alt=""> testimonial &nbsp;&nbsp;>>&nbsp;&nbsp; Add testimonial &nbsp;&nbsp;/&nbsp;&nbsp; Edit testimonial</h1></td>
...
<form action="" method="post" id="frmCreatestate" name="frmCreatestate" class="middle_form" enctype="multipart/form-data" onsubmit="return validate(this.image);">
	...
	<tr>
			 ...
			 <?php /*?> <tr><td colspan="2" height="7"></td></tr>
				...
				?>
				<img src="../uploads/testimonial/category/<?php echo $indivdata->image; ?>" width="185" height="79" />
              ...
              <tr>
				...
				<td width="69%"><?php if($indivdata->image!=""){
				?>
				<img src="../uploads/testimonial/thumbs/<?php echo $indivdata->image; ?>" width="150" height="150" />
				<input type="hidden" name="hdn_image" size="24" value="<?php echo $indivdata->image; ?>">
				<?php
				}
			  ...
			  <tr><td colspan="2" height="7"></td></tr>
               <tr>
                <td width="6%" align="left" class="caption-field"><label class="title">Name :</label></td>
                <td width="94%" align="left" valign="middle"><input name="name" class="text_large required" type="text" value="<?php echo  stripslashes($indivdata->name);?>" style="width:800px;"/><br />
                <small style="color:#FF0000">* (Provide Unique Slug Name )</small>                </td>
	</tr>
			  <tr><td colspan="2" height="7"></td></tr>
               <tr>
                <td width="6%" align="left" class="caption-field"><label class="title">Designation:</label> </td>
                <td width="94%" align="left" valign="middle"><input name="designation" class="text_large required" type="text" value="<?php echo  stripslashes($indivdata->designation);?>" style="width:800px;"/></td>
			  </tr>
