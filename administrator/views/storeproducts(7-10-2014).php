...
<form action="index.php?option=com_storeproducts_insert" method="post" id="frmCreatestate" name="frmCreatestate" class="middle_form" enctype="multipart/form-data" onsubmit="return validate();">
    ...
    <tr><td colspan="2" height="7"></td></tr>
           ...
           <tr>
                <td width="6%" align="left" class="caption-field"><label class="title">Monthly Packet Count :</label></td>
         ...
         <tr>
			  ...
			  <td width="7%" align="left" class="caption-field"><label class="title">Size:<span style=" color:#FF0000; font-size:14px">*</span></label></td>
                ...
                for(var i=0;i<document.getElementById('sid0').length;i++)
                {
				 ...
				 <input name="sid[<?php print $sid_single;?>]" class="inputtype_field_input" type="text" value="<?php echo $price->size; ?>" required="required" />
...
<?php
  ...
  $asq2="select * from tb_store_value where spid='".$indivdata->spid."' "; 
				         $price2=$callConfig->getALLRows($asq2);
...
?>
  ...
  <!--   <select name='sid[<?php print $s_price->id;?>]' id='sid<?=$i?>' class='select_medium'><option value=''>Select</option><?php print $optin; ?></select>-->
    <!--   <script type="text/javascript">
     ...
     <div class='clearfix'></div><a href='javascript:void(0);' onClick='removeFormField("#row<?=$i?>"),deleterow("#row<?=$i?>");return false;'>
...
$("#divTxt").append("<div id='row" + id + "'><div style='border:1px solid #6DB5FF;width:980px; height:45px;padding:2px 2px 2px 2px;'><p><select name='sid[]' id='sid"+id+"' class='select_medium'><option value=''>Select</option><?php print $optino; ?></select> price:<input type='text' name='rs[]' id='rs"+id+"' class='inputtype_field_input' /> Monthly Price:<input type='text' name='brs[]' id='brs"+id+"' class='inputtype_field_input' />  Weight(oz):<input type='text' name='wrs[]' id='wrs"+id+"' class='inputtype_field_input' />Quantity:<input type='text' name='qns[]' id='qns"+id+"' class='inputtype_field_input' /> </p><div class='clearfix'></div><a href='#' onClick='removeFormField(\"#row" + id + "\"); return false;'><img src='allfiles/Delete-Button_06.png' alt='delete' style='position:relative;top:-56px;float:right;right:-35px' /></a></div><br></div>");
$("label").inFieldLabels(); 
	id = (id - 1) + 2;
...
xmlhttp.send();
              ...
              <tr>
				...
				$oFCKeditor->Create() ;
				?><?php */?>
				<?php /*?><textarea name="bigtext" id="bigtext" cols="150" rows="4"><?php echo  stripslashes($indivdata->bigtext);?></textarea><?php */?>                </tr>
                <tr><td colspan="2" height="7"></td></tr>
			    <tr><td colspan="2" height="7"></td></tr>
			   <tr>
